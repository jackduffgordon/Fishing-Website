...
// Routes
app.get('/', (req, res) => {
  res.json({ message: 'Fishing Bookings API' });
});

// Example: Get all charters
app.get('/api/charters', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM charters');
...
// Example: Create a new charter
app.post('/api/charters', async (req, res) => {
  const { name, description, location, price_per_hour } = req.body;
  try {
    const result = await pool.query(
      'INSERT INTO charters (name, description, location, price_per_hour) VALUES ($1, $2, $3, $4) RETURNING *',
